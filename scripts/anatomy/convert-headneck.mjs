#!/usr/bin/env node
/**
 * Convertit un sous-ensemble Tête-et-Cou du jeu de données réel
 * BodyParts3D/Anatomography (DBCLS, CC BY-SA 2.1 Japan) — cloné en lecture
 * seule depuis https://github.com/Kevin-Mattheus-Moerman/BodyParts3D,
 * lui-même une conversion OBJ→STL binaire du jeu de données officiel
 * version 3.0 (20110915) — en fichiers glTF binaire (.glb), un par système
 * anatomique, pour être chargés indépendamment dans le visualiseur 3D.
 *
...
const CATALOG_PATH = path.join(REPO_ROOT, 'src', 'data', 'anatomy', 'headNeckCatalog.json');
...
function buildGlb(meshes, materials) {
  ...
  const gltfJson = {
    asset: {
      version: '2.0',
      generator: 'Musab Study — scripts/anatomy/convert-headneck.mjs',
      copyright:
